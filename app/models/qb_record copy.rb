  ...
  def self.can_create? u, *args
	...
	module HasPath
		...
		included {
			...
			before_validation { |o|				
				o.sync_path if o.parent_id_changed?
				o.sync_full_path if o.name_changed?
			}
			after_create { |o|
				o.sync_full_path
	...
	def self.update_transaction_balance
		DB.query('update qb_transactions t set t.balance = (
			select sum(d.amount) from qb_transaction_details d where d.payment_id is null and d.amount != 0 and d.qb_transaction_id = t.id
		)')
	end
	
	def self.update_customer_balance
		DB.query('update qb_customers c set c.balance = (
			select sum(d.amount) from qb_transaction_details d where d.payment_id is null and d.amount != 0 and d.voided = 0 and d.qb_customer_id = c.id
		)')
	end
